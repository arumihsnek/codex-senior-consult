...
The historical response contract is version `codex-senior-consult-response/v2`. It is mode-specific, minimal, strict (`additionalProperties: false`), and does not require the target model to copy wrapper metadata. V2 response handling is retained only for explicit historical compatibility; v1 responses remain available only through the explicit transition represented by a v1 bundle request or `--legacy-response-v1`. New ordinary consultations use v3; see the migration notes in [references/contracts.md](references/contracts.md).
